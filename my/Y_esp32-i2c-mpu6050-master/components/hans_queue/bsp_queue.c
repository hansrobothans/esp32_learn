...
void bsp_queue_tcp_recive_send_to_oled(void * pvParameters)
{
    ...
    #ifdef BSP_TCP_H
    {
		...
		bsp_tcp_recive_message bsp_tcp_recive_message_v;
		// 发给oled_rgb队列消息变量
		bsp_oled_message oled_message_send = {0,{0}};
		while(1)
		{
		// 接受数据
		xResult = xQueueReceive(bsp_tcp_recive_xQueue,(void *)(&bsp_tcp_recive_message_v),( TickType_t ) 10 ) ;
		strcpy(oled_message_send.data, bsp_tcp_recive_message_v.data);
		// 判断是否接受数据成功
		if(xResult == pdPASS)
		{
		  printf("接收到消息队列数据oled_chr_get1 = %s\r\n", bsp_tcp_recive_message_v.data);
		  // 将接收到的数据发送出去
		  xQueueSend(bsp_oled_xQueue,(void *) &oled_message_send,( TickType_t ) 10);
		}
		// vTaskDelay(1000 / portTICK_PERIOD_MS);
		} 
    }
...
void bsp_queue_tcp_recive_send_to_oled_task(void * pvParameters)
{
  ...
  xTaskCreate(
      &bsp_queue_tcp_recive_send_to_oled,/* 任务函数 */
      "bsp_queue_tcp_recive_send_to_oled",/* 任务名称 */
      3072,/* 任务的堆栈大小 */
      NULL,/* 任务的参数 */
      5,/* 任务的优先级 */
      NULL);/* 跟踪创建的任务的任务句柄 */
...
*               Ver0.0.1:
                    hans, 2019/09/04, 初始化版本\n 
*/
void bsp_tcp_recive_send_to_led_task(void * pvParameters)
{
  // 创建tcp接受消息转发给led的功能任务
  xTaskCreate(
      &bsp_queue_tcp_recive_send_to_led,/* 任务函数 */
      "bsp_queue_tcp_recive_send_to_led",/* 任务名称 */
      3072,/* 任务的堆栈大小 */
      NULL,/* 任务的参数 */
      5,/* 任务的优先级 */
      NULL);/* 跟踪创建的任务的任务句柄 */
