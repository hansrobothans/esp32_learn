...
void PCD_Init(spi_device_handle_t spi){
    ...
    PCD_Version(spi);
    gpio_pad_select_gpio(BSP_MFRC522_RST);
    gpio_set_direction(BSP_MFRC522_RST, GPIO_MODE_OUTPUT);

    // Hard Reset RFID
    gpio_set_level(BSP_MFRC522_RST, 0);
    vTaskDelay(50 / portTICK_PERIOD_MS);
    gpio_set_level(BSP_MFRC522_RST, 1);
    vTaskDelay(50 / portTICK_PERIOD_MS);
...
void bsp_mfrc522_init(void)
{
    printf("haha\n");

    esp_err_t ret;
    spi_bus_config_t bsp_mfrc522_buscfg={
        .miso_io_num=BSP_MFRC522_MISO,
        .mosi_io_num=BSP_MFRC522_MOSI,
        .sclk_io_num=BSP_MFRC522_CLK,
        .quadwp_io_num=-1,
        .quadhd_io_num=-1
    };
    spi_device_interface_config_t bsp_mfrc522_devcfg={
        .clock_speed_hz=5000000,               //Clock out at 5 MHz
        .mode=0,                                //SPI mode 0
        .spics_io_num=BSP_MFRC522_CS ,               //CS pin
        .queue_size=7,                          //We want to be able to queue 7 transactions at a time
        //.pre_cb=ili_spi_pre_transfer_callback,  //Specify pre-transfer callback to handle D/C line
    };
    spi_device_handle_t spi;
    //Initialize the SPI bus
    ret=spi_bus_initialize(HSPI_HOST, &bsp_mfrc522_buscfg, 1);
    assert(ret==ESP_OK);
    //Attach the RFID to the SPI bus
    ret=spi_bus_add_device(HSPI_HOST, &bsp_mfrc522_devcfg, &spi);
    assert(ret==ESP_OK);
